...
JAX Implementation optimized for NVIDIA CUDA GPUs
...
- Bilinear (Tustin) discretization with Discretization Guard
- Error-compensated recurrence via trapezoidal input averaging
- Hardware-aware parallel scan using jax.lax.associative_scan

Author: Implementation for CUDA backend
"""
...
def check_device():
    ...
    Checks for NVIDIA (CUDA) -> Apple Silicon (Metal) -> CPU fallback.
    """
    print(f"JAX version: {jax.__version__}")
    
    # 1. Try NVIDIA CUDA
    try:
        # We explicitly ask for 'cuda' or 'gpu' to target NVIDIA hardware
        cuda_devices = jax.devices('cuda')
        print(f"✓ Target Backend: CUDA (NVIDIA)")
        print(f"  Devices: {cuda_devices}")
        return 'cuda'
    except RuntimeError:
        pass  # Fall through to check Metal

    # 2. Try Apple Metal (MPS)
    try:
        # 'metal' backend requires the jax-metal package installed
        metal_devices = jax.devices('metal')
        print(f"✓ Target Backend: Metal (Apple M-Series)")
        print(f"  Devices: {metal_devices}")
        print("  > Note: Ensure 'jax-metal' is installed.")
        return 'metal'
    except RuntimeError:
        pass  # Fall through to CPU

    # 3. Fallback to CPU
    print("! Warning: No Accelerator found. Running on CPU.")
    print(f"  Available devices: {jax.devices()}")
    return 'cpu'


# =============================================================================
# Phase 1: Discretization Functions
# =============================================================================
...
def soft_clamp(
    ...
    Maps input smoothly into [min_val, max_val]. Behaves approximately
    linearly near the center of the range and saturates smoothly at
    the boundaries, preserving gradient flow.

    Args:
        x: Input tensor of any shape.
        min_val: Lower bound (epsilon). Default 1e-4 to prevent
                 division by zero in the Tustin denominator.
        max_val: Upper bound (Δ_max).

    Returns:
        Clamped tensor with same shape as x, values in [min_val, max_val].
    """
...
def discretize_tustin(
    ...
    Converts continuous-time SSM parameters to discrete-time using
    the Tustin (bilinear) transformation, which is a second-order
    Padé approximation of the matrix exponential.

    For diagonal A with elements a_i, the full matrix formulas:
        Ā = (I - Δ/2 · A)^{-1} (I + Δ/2 · A)
        B̄ = (I - Δ/2 · A)^{-1} √Δ · B

    reduce to element-wise operations:
        ā_i = (1 + Δ/2 · a_i) / (1 - Δ/2 · a_i)
        b̄_i = √Δ · b_i / (1 - Δ/2 · a_i)

    Precision Guard: All intermediate computations are promoted to
    float32 to prevent bfloat16 underflow in the inversion step,
    then cast back to the original dtype.

    Associative Scan Compatibility:
        The resulting (Ā, B̄) produce a recurrence of the form
        h_t = ā_t · h_{t-1} + b̄_t which remains associative:
            (a₂, b₂) ⊗ (a₁, b₁) = (a₂·a₁, a₂·b₁ + b₂)
        so jax.lax.associative_scan can still parallelize over L.

    Args:
        A: Continuous state matrix (diagonal), shape (D, N).
           Expected to be negative for stable dynamics.
        B: Continuous input matrix, shape (B, L, D, N).
        delta: Step sizes after discretization guard,
               shape (B, L, D), values in [ε, Δ_max].
    ...
    # Half-step: Δ/2 · A  (element-wise, both terms broadcast)
    half_dA = (delta_f32 / 2.0) * A_f32  # (B, L, D, N)

    # Denominator: (1 - Δ/2 · A)
    # Since A < 0 and delta > 0, half_dA < 0, so denom > 1. Safe.
    ...
    # Cast back to original dtype (no-op when already float32)
...
def selective_ssm(
    ...
    2. Guard Δ with soft_clamp
    3. Discretize using Bilinear (Tustin) transformation
    4. Compute scan with trapezoidal input averaging
    5. Add skip connection

    Note: This standalone function does not include the learnable
    RMSNorm guard (requires Flax module context). For the full
    guarded pipeline, use S6Layer.
    ...
    Args:
        ...
        delta_max: Upper bound for discretization guard
    ...
    # === Discretization Guard (soft clamp only, no RMSNorm) ===
    delta = soft_clamp(delta, min_val=1e-4, max_val=delta_max)

    # === Discretization (Bilinear / Tustin) ===
    A_bar, B_bar = discretize_tustin(A, B, delta)

    # === Error-Compensated Recurrence (Trapezoidal Averaging) ===
    ...
    # === Selective Scan ===
    if use_parallel:
        y, _ = selective_scan_parallel(A_bar, B_bar, C, x_trap)
    else:
        y, _ = selective_scan_sequential(A_bar, B_bar, C, x_trap)
...
class CausalConv1D(nn.Module):
    ...
    Note: Implemented manually for maximum compatibility across backends.
    CUDA backend supports grouped convolutions natively for optimal performance.
    """
    features: int
    kernel_size: int = 4
...
class S6Layer(nn.Module):
    ...
    Uses Bilinear (Tustin) discretization with a Discretization Guard
    ...
    - "vanilla": Standard ZOH discretization without guards (for baseline comparison)
    """
    ...
    delta_max: float = 10.0  # Upper bound for Discretization Guard soft clamp
    mode: str = "tustin"  # "tustin" or "vanilla" (baseline)
    ...
    def __call__(
        ...
        if self.mode == "vanilla":
            # Vanilla baseline: NO discretization guard
            ...
            #    ε = dt_init_floor (1e-4) prevents division by zero in Tustin
            #    Δ_max caps OOD step sizes
        ...
        if self.mode == "vanilla":
            # Vanilla baseline: Standard ZOH discretization
            # - No float32 precision guard (runs in model's default precision, likely bfloat16)
            # - Uses exp(ΔA) instead of Tustin's rational approximation
            A_bar, B_bar = discretize_zoh(A, B_expanded, delta)
        else:
            # Tustin mode: Bilinear (Tustin) discretization
            # - Uses (I ∓ Δ/2·A) rational mapping instead of exp(ΔA)
        ...
        if self.mode == "vanilla":
            # Vanilla baseline: Raw input (no trapezoidal averaging)
            # Standard SSM: h_t = Ā_t · h_{t-1} + B̄_t · x_t
            x_input = x  # (B, L, D)
        else:
            # Tustin mode: Error-Compensated Recurrence (Trapezoidal Averaging)
            # Tustin is a second-order (trapezoidal) approximation, so the
            # state update should use the midpoint input:
            #   h_t = Ā_t · h_{t-1} + B̄_t · (x_t + x_{t-1}) / 2
            # x_{-1} is taken as zero (no future information leakage).
            x_prev = jnp.concatenate(
                [jnp.zeros_like(x[:, :1, :]), x[:, :-1, :]], axis=1
            )  # (B, L, D)
        ...
        # === Selective Scan ===
        # The recurrence h_t = a_t·h_{t-1} + b_t remains associative,
        # so jax.lax.associative_scan can still parallelize over L.
        if use_parallel:
...
def test_discretization():
    """Test Tustin (Bilinear) discretization and soft_clamp guard."""
    print("\n" + "="*60)
    print("Testing Tustin (Bilinear) Discretization")
    print("="*60)
    ...
    delta_raw = jax.nn.softplus(jax.random.normal(subkey, (B_size, L, D)) - 2)

    # --- Test soft_clamp ---
    delta = soft_clamp(delta_raw, min_val=1e-4, max_val=10.0)
    print(f"Delta raw range: [{delta_raw.min():.6f}, {delta_raw.max():.6f}]")
    print(f"Delta clamped range: [{delta.min():.6f}, {delta.max():.6f}]")
    assert jnp.all(delta >= 1e-4), "soft_clamp lower bound violated"
    assert jnp.all(delta <= 10.0), "soft_clamp upper bound violated"
    print("✓ soft_clamp guard passed!")
    ...
    # For Tustin with A < 0: A_bar = (1 + dA/2) / (1 - dA/2)
    # Since dA/2 < 0, numerator < 1 and denominator > 1, so |A_bar| < 1
    print(f"A_bar range: [{A_bar.min():.4f}, {A_bar.max():.4f}]")
    print(f"Delta range: [{delta.min():.4f}, {delta.max():.4f}]")

    # Verify stability: |A_bar| < 1 for negative A (Tustin preserves stability)
    assert jnp.all(jnp.abs(A_bar) < 1.0), "Tustin A_bar should be bounded by 1"

    # --- Verify precision: output should be float32 ---
    assert A_bar.dtype == jnp.float32, f"Expected float32, got {A_bar.dtype}"
    assert B_bar.dtype == jnp.float32, f"Expected float32, got {B_bar.dtype}"
    print("✓ Precision guard: outputs are float32")

    # --- Compare with ZOH for small delta (should be similar) ---
    A_bar_zoh, B_bar_zoh = discretize_zoh_simple(A, B, delta)
    a_diff = jnp.max(jnp.abs(A_bar - A_bar_zoh))
    print(f"\nTustin vs ZOH A_bar max diff: {a_diff:.4e}")
    print("  (Expected small for small delta, diverges for large delta)")

    print("✓ Tustin discretization test passed!")
    return True
